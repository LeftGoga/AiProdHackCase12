        ...
        if extension in ['.doc', '.docx']:
            ...
            for page in range(0,page_count):
                ...
                with open(log_file_path, 'a', encoding='utf-8') as log_file:
                    log_file.write(f"{current_time} - Страница {page + 1} из {page_count} ---- {file_path}\n")
                ...
                for i in range(len(tables_list)):
                    ...
                    for j in range(len(tables_list[i]['page_coords'])):             
                        if(flag_continued_table == True):
                            flag_continued_table = False
                            temp_promt = ''
                            continue

                        image_to_summary+=[table_extractor.extract_table_image(file_path, page+1+j, tables_list[i]['page_coords'][j])]   

                    temp_promt = table_extractor.extract_table_text(file_path, page+1, page_count)[i]
                    promt = temp_promt[0:len(temp_promt)//2]
                        
                    if len(image_to_summary)!=0:
                        # print("ПРОМТ OCR: ", i, j, promt)
                        pass
                        summary = self.llama_cpm.generate_summary(image_to_summary, promt)
                        # print(i, j, len(image_to_summary), summary)
                    ...
                    if(i == len(tables_list)-1 and len(tables_list[i]['page_coords'])>1):

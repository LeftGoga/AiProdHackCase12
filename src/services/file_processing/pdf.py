...
class PDFProcessor:
    ...
    def json_converter(
        self, raw_text: str, file_path: str, page_number: int, doc_type="text"
    ) -> dict:
        """
    ...
    def process_pdf_page(
        self, pdf_path: str, page_number: int, page_count: int
    ) -> dict:
        """
        ...
        if self.is_scanned_pdf(pdf_path):
            ...
            if (
                len(
                    self.table_extractor.extract_tables_from_pdf(
                        pdf_path, page_number, page_count, combine_tables=False
                    )
                )
                != 0
            ):
            ...
            filtered_text = pytesseract.image_to_string(
                image_without_tables, lang="rus+eng"
            )
        else:
            raw_text = pytesseract.image_to_string(image, lang="rus+eng")

            if raw_text and raw_text.strip():
                tables_ocr_text = self.table_extractor.extract_table_text(
                    pdf_path, page_number - 1, page_count
                )
    ...
    def remove_tables_from_text(self, raw_text, tables):
        ...
        for table in tables:
            ...
            start_idx, end_idx = self.find_table_boundaries(
                raw_text, clean_text_lines, table_ocr_lines
            )

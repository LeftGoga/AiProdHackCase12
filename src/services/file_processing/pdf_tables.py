...
class PDFTableExtractor:
    ...
    def extract_tables_from_pdf(
        self, pdf_path, page_num, page_count, combine_tables=False
    ):
        page_num = page_num - 1
        ...
        if combine_tables and page_num < len(pdf_document) - 1 and tables_json:
            next_page_tables_json = self.extract_tables_on_page(
                pdf_document, pdf_path, page_num + 1
            )
            if next_page_tables_json:
                for current_table in tables_json:
                    for next_table in next_page_tables_json:
                        if self.is_table_continued(
                            current_table, next_table, page_height, next_page_height
                        ):
                            current_table["page_coords"].extend(
                                next_table["page_coords"]
                            )
        ...
        with io.BytesIO() as image_buffer:
            ...
            try:
                ...
                for table in extracted_tables:
                    min_x = min(
                        cell.bbox.x1 for row in table.content.values() for cell in row
                    )
                    max_x = max(
                        cell.bbox.x2 for row in table.content.values() for cell in row
                    )
                    min_y = min(
                        cell.bbox.y1 for row in table.content.values() for cell in row
                    )
                    max_y = max(
                        cell.bbox.y2 for row in table.content.values() for cell in row
                    )
    ...
    def is_table_continued(
        self, current_table, next_table, page_height, next_page_height
    ):
    ...
    @staticmethod
    def extract_table_image(pdf_path, page_num, coordinates):
        page_num = page_num - 1
        pdf_document = fitz.open(pdf_path)
        page = pdf_document.load_page(page_num)
